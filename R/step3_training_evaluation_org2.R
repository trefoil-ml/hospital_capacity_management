...
boosted_model <- rxDForest(formula = formula,
                          data = LoS_Train,
                          nTree = 50,
                          minSplit = 10,
                          minBucket = 5,
...
# Save the Random boosted in SQL. The compute context is set to local in order to export the model. 
rxSetComputeContext(local)
...
# Compute the performance metrics of the model.
boosted_prediction <- rxImport(inData = boosted_prediction_sql)
